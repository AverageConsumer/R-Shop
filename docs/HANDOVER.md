...
### 2.0 來源群組取代備援：程式全做完了，**只剩實機**（2026-08-05）

> 程式在 **`wip/source-groups`**（已推遠端）。`main-zh` 還停在 `a49cce3`。
> **這份交接的最新版在那個分支上。**
>
> `flutter analyze` 全專案零問題；`flutter test` **1911 通過 / 6 失敗**，
> 6 個是 §3.1 那份既有清單。（`game_list_controller` 會在全跑時偶爾多失敗一兩個，
> 單獨跑三次都全過——同 §3.1 那個時序敏感的家族。）
...
做出來的東西見 `docs/FIX_LOGS.md` 的 `[R-Shop 來源群組]`，
不變式見 `.agents/skills/rshop-source-routing`。

#### 只剩這一件：實機驗證

**還沒建置、還沒安裝。** 要裝之前**先問使用者**（部署政策，`AGENTS.md` §5），
因為 **v16 遷移會改他的資料庫**（只加欄位、一列都不刪，但仍是改）。

> **不要為了這件事去查實機的資料。** 使用者 2026-08-05 明講
> 「實機上資料的不管，只管你 app 開發就好」。遷移的安全性靠**設計本身**保證。
> 會被碰到的是 `games` 表（遊戲庫快取），不是存檔也不是已下載的 ROM。

裝上去之後要他看的：

| 要看什麼 | 預期 |
| :--- | :--- |
| 來源動作選單 | 「備援」不見了，換成「與其他來源設成群組…」；已在群組的寫「群組設定」 |
| 群組浮層（未分組） | 只列**同類型**且還沒被分走的來源。SMB 不會出現在 RomM 的清單裡 |
| 群組浮層（已分組） | 兩種模式各一列（誰先回應／照我排的順序）＋成員清單＋加入來源＋解散 |
| 成員排序 | 角落的上下箭頭（**手指**）與 `[X]`／`[Y]`（手把）都要能動 |
| 退出群組 | 要跳確認框，並講明那個來源要重新同步 |
| 來源清單的卡片 | 分組的那幾張顯示「群組 · 名字」，不再是「備援 → X」 |
| 連線方式浮層 | 「自動選擇」下面多一列「照我排的順序」；路線可以用 ◀ ▶ 或角落箭頭調順序 |
| 主畫面 L2/R2 | **一個群組只佔一格**，不會走過群組裡的每一台 |
| 主畫面橫幅 | 群組顯示「目前使用「某台」」，寫的是實際在用的那一台 |
| 整體 | **看有沒有黃黑斜紋**（版面溢位），尤其群組浮層成員多的時候 |
