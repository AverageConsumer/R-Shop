...
### 2.0 進行中：路線各自驗證、清單一個來源一份、自動選最優路線

**這一項推翻了不變式 5，不是繞過它。** 使用者 2026-08-04 確認的前提：
同一個來源底下的多條連線方式**就是同一台伺服器**，但**各自需要驗證**
（區網直連與 DDNS 走不同的前門）。

整個設計就兩句：**憑證跟著路線走，清單跟著來源走。**

| 部分 | 內容 |
| :--- | :--- |
| 路線憑證 | `SourceEndpoint` 有自己的可選 auth，沒設就沿用 `Source.auth`（舊設定檔零遷移）。`withLiveEndpoint()` 要把 auth 一起寫上頂層以維持不變式 4；**`connectionKey` 必須把憑證算進去**，否則換路線換 token 會重用錯的連線 |
| schema v15 | 唯一索引拿掉 `endpoint_id`，變成 `(systemSlug, filename, source_id)`。欄位保留當「上次從哪條路抓的」。**遷移要把 v14 的重複列去重，保留已下載到裝置上的那筆**——這是唯一有資料風險的一步 |
| 自動選路 | 探測改成回延遲並排序，能通的裡面挑最快。`pin` 從「選定」變成「覆寫」 |
| UI 與七語系 | 連線方式編輯頁加憑證欄位、選擇浮層顯示延遲與自動／釘選。`sources_routeSameServerHint` 的意思變了（不再是 401 警告） |

為什麼不是「讀取時合併兩份清單」：那是在補一個不該存在的分裂。
**程式分不出兩個位址是不是同一台，也不該去猜**（曾從埠號推論過，推論正確但不算數）——
能宣告的只有使用者。他宣告了，清單就是一份。

先前記在 `[R-Shop 自動選最快]` 的「決定不做」**已被這一項取代**，
做完要回去改寫那條，不要留著兩個相反的結論。
