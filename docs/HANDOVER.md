...
> 這一項未完成的程式全部 commit 在 **`wip/source-groups`（`cedce1e`，已推遠端）**。
> **那個分支整體不能編譯**——群組那一半做到一半。要接手就 `git checkout wip/source-groups`。
...
#### 已完成（在 wip 分支上，測試綠）
    ...
    測試                source_endpoint 與 endpoint_probe_service 兩檔共 90 項全過

#### 還沒做（**下次從這裡開始**）

1. **`database_service.dart` 卡在半路** —— 停在「v15 重構完、v16 遷移還沒寫」。
   要做的是**把快取的擁有者從來源改成群組**（有群組就歸群組）。
   偏好**加明確欄位**而不是共用某個代表 id——這個表已經吃過「來源歸屬埋在 `provider_config` JSON 裡」的虧。
   去重判準**照抄 v15 的 `_v15OnDeviceRank`**，不要另發明。整件事要在一個交易裡。
...
9. 全部做完才實機驗證。**v16 遷移會改使用者的資料庫**（他機器上約十萬列，v15 那次去重了 8534 列），
   裝上去之前先問他。
