...
| 關鍵字 | 症狀 / 根因 | 主要動到的檔案 |
| :--- | :--- | :--- |
| **使用中與顯示分家** ⚠️ | ① `??=` 種值表達不了「刻意是 null」，取消要按兩次。② **使用中**（同步＋預設顯示，`primarySourceId`）與**顯示**（主畫面 L2/R2，`activeSourceId`）拆成兩件事；同步改讀 primary。舊設定檔靠 `?? activeSourceId` 回填，無遷移 | `lib/models/config/app_config.dart`（`primarySourceId`） · `lib/services/sources_notifier.dart`（`setPrimarySource`） · `lib/services/source_failover.dart`（`resolveForSync`） · `lib/features/settings/sources_screen.dart` · `lib/features/home/home_view.dart` |
